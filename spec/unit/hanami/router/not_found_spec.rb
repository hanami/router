...
RSpec.describe Hanami::Router do
  ...
  describe "Not Found" do
    it "it returns 404 when an endpoint cannot be found" do
      ...
      expect(headers).to eq(RSpec::Support::HTTP.headers({"Content-Length" => "9"}))
    ...
    context "with not_found option" do
      let(:not_found) { ->(*) { [499, RSpec::Support::HTTP.headers({"Content-Type" => "application/json"}), [JSON.dump({error: "not_found"})]] } }
